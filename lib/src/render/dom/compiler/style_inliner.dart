library angular2.src.render.dom.compiler.style_inliner;
...
class StyleInliner {
  ...
  StyleInliner(this._xhr, this._styleUrlResolver, this._urlResolver) {}
  /**
...
var _importRe = new RegExp(r'@import\s+([^;]+);');
var _urlRe = RegExpWrapper
    .create("url\\(\\s*?['\"]?([^'\")]+)['\"]?|" + "['\"]([^'\")]+)['\"]");
var _mediaQueryRe =
    new RegExp(r'[' + "'" + r'"][^' + "'" + r'"]+[' + "'" + r'"]\s*\)?\s*(.*)');
