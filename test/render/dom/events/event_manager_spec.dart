...
main() {
  ...
  describe("EventManager", () {
    it("should delegate event bindings to plugins", () {
      ...
      expect(plugin._nonBubbleEventHandlers["click"]).toBe(handler);
    });
    it("should delegate bubbling events to plugins", () {
      ...
      var manager =
          new EventManager([plugin, domEventPlugin], new FakeNgZone());
      manager.addEventListener(element, "^click", handler);
      expect(plugin._bubbleEventHandlers["click"]).toBe(handler);
    });
    it("should delegate event bindings to the first plugin supporting the event",
      ...
      expect(plugin1._nonBubbleEventHandlers.containsKey("click")).toBe(false);
      expect(plugin2._nonBubbleEventHandlers["click"]).toBe(clickHandler);
      expect(plugin2._nonBubbleEventHandlers.containsKey("dblclick"))
          .toBe(false);
      expect(plugin1._nonBubbleEventHandlers["dblclick"]).toBe(dblClickHandler);
    });
...
class FakeEventManagerPlugin extends EventManagerPlugin {
  ...
  Map<String, Function> _nonBubbleEventHandlers;
  Map<String, Function> _bubbleEventHandlers;
  ...
  addEventListener(
    ...
    if (shouldSupportBubble) {
      this._bubbleEventHandlers[eventName] = handler;
    } else {
